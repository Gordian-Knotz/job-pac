...
export function JobCard({
   ...
   * benefit-tag pill row (remote + top required skills). Every other call
   * site (homepage, related jobs, saved jobs) omits this and keeps today's
   * shape exactly.
   */
  variant?: "default" | "compact";
  /** Compact-variant only: highlights the card as split view's current selection. */
  active?: boolean;
  /**
   * Compact-variant only: swaps the card's navigating <Link> for a click
   * handler that selects the job in split view instead of leaving the page.
   * Cards without this still navigate to /jobs/[slug] as normal.
   */
  onSelect?: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none relative z-[1] pr-9">
        ...
        {(skillPills.length > 0 || (compact && job.is_remote)) && (
          <div className="mt-3 flex flex-wrap items-center gap-1.5">
            {compact && job.is_remote && (
